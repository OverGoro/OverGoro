...
int main(int argc, char **argv)
{
    ...
    film_t *film_array = NULL;
    size_t n = 0;
    
    if (strcmp(argv[2], FIELD_TYPE_TITLE) == 0)
        rc = fa_sorted_create(f_r, &film_array, &n, film_cmp_by_title);
    else if (strcmp(argv[2], FIELD_TYPE_NAME) == 0)
        rc = fa_sorted_create(f_r, &film_array, &n, film_cmp_by_name);
    else if (strcmp(argv[2], FIELD_TYPE_YEAR) == 0)
        rc = fa_sorted_create(f_r, &film_array, &n, film_cmp_by_year);
    ...
    else if (argc == 4 && rc == ERR_OK)
    {
        ...
        film_t prototype = { NULL, NULL, 0 };

        // Поиск по названию
        if (strcmp(argv[2], FIELD_TYPE_TITLE) == 0)
        {
            prototype.title = argv[3];
            found_film = fa_bin_search(film_array, n, &prototype, film_cmp_by_title);
        }
        // Поиск по фамилии автора
        else if (strcmp(argv[2], FIELD_TYPE_NAME) == 0)
        {
            prototype.name = argv[3];
